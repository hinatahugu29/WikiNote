...
- **※ データに変更がない場合は、バックアップ作成（およびファイルの上書き）をスキップします。**
...
**方法1: バックアップ管理画面から作成（推奨）**
「📦 バックアップ管理」を開き、「📸 今すぐバックアップ」ボタンをクリックします。現在の状態が即座にバックアップファイルとして保存されます。
...
### ⚠️ 記事が表示されない / 画面が真っ白になる

1. サーバー接続エラーの可能性があります。ターミナル（黒い画面）が開いているか確認してください。
2. ターミナルが閉じている場合は、`start.bat` をダブルクリックして再起動してください。
3. `data/wiki_data.json` が何らかの理由で破損している可能性があります。「📦 バックアップ管理」から直前のバックアップを復元してください。

### ⚠️ バックアップファイルが空（0KB）になる

- **Ver 2.1以降で修正済み**: データの整合性チェック機能が追加され、不正なデータ（空データなど）による上書きやバックアップ作成を防止するようになりました。
- 万が一発生した場合は、手動で `backups/` フォルダ内の正常なサイズ（数KB以上）のファイルを探し、リストから復元してください。
...
**機能改善:**
- 🖼️ **サイドバーの機能強化**: ドラッグでの幅調整（リサイズ）に対応し、幅を保存するようにしました。
- 🔄 **スクロール同期**: エディタとプレビューのスクロール位置を双方向に同期。ツールバーの `🔗` で切替可能。
- 🚀 **UX改善**:
    - クイック移動時に現在のモード（編集/プレビュー）を維持するように改善。
    - プレビューから戻る際の挙動をスムーズに。
    - 目次（TOC）リンククリック時のスクロール位置修正。
    - 記事切り替え時にスクロール最上部へリセット。
- 🕸️ **グラフビュー**: コントロールボタンの自動フェードアウト機能を追加。
